...
import com.cibertec.edu.Repository.PacientesRepository;
...
public class pacienteController {
    ...
    @PostMapping("/guardar")
    public String guardarPaciente(@ModelAttribute Pacientes paciente, Model model) {
        pacienteService.guardarPaciente(paciente); // Llama al servicio para guardar el paciente
        model.addAttribute("message", "Paciente guardado correctamente");
    ...
    public String editarPaciente(@PathVariable Integer id, Model model) {
        ...
        public String actualizarPaciente(@ModelAttribute Pacientes paciente) {
            // Ensure that dni is not null before saving
            if (paciente.getIdPaciente() == null) {
                throw new IllegalArgumentException("El ID es obligatorio.");
            }
            // Save the updated patient
            pacientesRepository.save(paciente);
            // Redirect to another page or return the response
    ...
    public String mostrarDetallePaciente(@PathVariable("id") Integer id, Model model) {
        Optional<Pacientes> pacienteOpt = pacienteService.buscarPorId(id);
    ...
    public ResponseEntity<?> buscarPorId(@PathVariable Integer id) {
        Optional<Pacientes> paciente = pacienteService.buscarPorId(id);
    ...
    // Este es el método que debes modificar/reemplazar
    @GetMapping("/obtenerPacientePorId") // La URL ya es correcta
    @ResponseBody // Añade esta anotación para que Spring serialice el objeto Pacientes a JSON
    public ResponseEntity<Pacientes> obtenerPacientePorId(@RequestParam Integer id) {
        // Asumiendo que pacienteService.buscarPorId(id) devuelve Optional<Pacientes>
        // Si tu método en el servicio es diferente (ej. findById), úsalo aquí.
        Optional<Pacientes> pacienteOpt = pacienteService.buscarPorId(id);

        if (pacienteOpt.isPresent()) {
            return ResponseEntity.ok(pacienteOpt.get()); // Devuelve el objeto Pacientes completo
        } else {
            // Devuelve 404 NOT_FOUND si el paciente no existe
